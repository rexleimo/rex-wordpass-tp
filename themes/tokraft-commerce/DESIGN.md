...
## Tokraft commerce adaptation

This theme uses the reference as a motion and merchandising baseline, not as a
brand imitation. Tokraft keeps its existing deep-navy, electric-blue,
safety-gold, white, and precision-grid identity. Product and production imagery
remain the visual focus, while interface chrome stays flat, technical, and
legible.

- Motion is purposeful: one orchestrated hero entrance, staggered section
  reveals, subtle media parallax, and restrained magnetic primary actions.
- GSAP motion is limited to the front page and must never block commerce,
  carousel, form, cart, or checkout behavior.
- `prefers-reduced-motion: reduce` disables all GSAP enhancement.
- Avoid ornamental bouncing, infinite motion, excessive blur, and generic
  gradient decoration. The page should feel engineered, not playful.
- Preserve the theme's existing type, spacing, buttons, colors, and responsive
  breakpoints unless a task explicitly requests a visual redesign.
